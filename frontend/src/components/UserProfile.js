...
import { Container, Typography, TextField, Button, Box, Alert, CircularProgress, Select, MenuItem, InputLabel, FormControl, Divider } from '@mui/material';
...
const UserProfile = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    if (editableProfile.name !== profile.name) {
      updatePayload.name = editableProfile.name;
    }
    if (editableProfile.username !== profile.username) {
      updatePayload.new_username = editableProfile.username;
    }
    if (editableProfile.email !== profile.email) {
      updatePayload.new_email = editableProfile.email;
    }
    if (editableProfile.phone_number !== profile.phone_number) {
      updatePayload.phone_number = editableProfile.phone_number;
    }
    if (editableProfile.date_of_birth !== profile.date_of_birth) {
      updatePayload.date_of_birth = editableProfile.date_of_birth;
    }
    if (editableProfile.authentication_method !== profile.authentication_method) {
      updatePayload.new_authentication_method = editableProfile.authentication_method;
    }
    if (newPassword) {
      updatePayload.new_password = newPassword;
    }
    if (addToWallet) {
      updatePayload.add_to_wallet_balance = parseInt(addToWallet);
    }
    ...
    try {
      ...
      setEditableProfile(response.data.user_info);
      setNewPassword('');
      setAddToWallet('');
    ...
    <Container maxWidth="sm" sx={{ mt: 5 }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        />
         <TextField
          ...
          value={`${profile.wallet_balance || 0} تومان`}
          disabled
        />
         <TextField
          label="روش احراز هویت"
          value={profile.authentication_method || ''}
          disabled
        />
      </Box>
      ...
      <Box component="form" onSubmit={handleUpdate} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <TextField
          ...
          onChange={handleEditableChange}
          helperText="در صورت تغییر، باید دوباره وارد شوید."
        />
        <TextField
          name="email"
          label="ایمیل جدید"
        ...
        />
        <TextField
          name="add_to_wallet_balance"
          label="افزایش موجودی کیف پول"
          type="number"
          value={addToWallet}
          onChange={(e) => setAddToWallet(e.target.value)}
        />
        <FormControl fullWidth>
          <InputLabel>روش احراز هویت جدید</InputLabel>
          <Select
            name="authentication_method"
            value={editableProfile.authentication_method || ''}
            onChange={handleEditableChange}
            label="روش احراز هویت جدید"
          >
